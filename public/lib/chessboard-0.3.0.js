...
; (function () {
    ...
    window['ChessBoard'] = window['ChessBoard'] || function (containerElOrId, cfg) {
        ...
        var CSS = {
            ...
            //highlight2_target: 'highlight2_target',
            highlight_select: 'highlight_select',
            //highlight2_select: 'highlight2_select',
        ...
        */
        ...
        function beginDraggingPiece(source, piece, x, y) {
            boardEl.find('.' + CSS.square).removeClass(CSS.highlight_select);
            boardEl.find('.' + CSS.square).removeClass(CSS.highlight_target);
            boardEl.find('.' + CSS.square).removeClass(CSS.highlight_source);
        ...
        */
        ...
        widget.highlight = function (source) {
            $('#' + SQUARE_ELS_IDS[source]).addClass(CSS.highlight1);
        };

        widget.highlight_source = function (source) {
            boardEl.find('.' + CSS.square).removeClass(CSS.highlight_select);
            boardEl.find('.' + CSS.square).removeClass(CSS.highlight_target);
            boardEl.find('.' + CSS.square).removeClass(CSS.highlight_source);
            $('#' + SQUARE_ELS_IDS[source]).addClass(CSS.highlight_source);
        };

        widget.highlight_target = function (target) {
            boardEl.find('.' + CSS.square).removeClass(CSS.highlight_select);
            boardEl.find('.' + CSS.square).removeClass(CSS.highlight_target);
            $('#' + SQUARE_ELS_IDS[target]).addClass(CSS.highlight_target);
        };

        widget.highlight_select = function (select) {
            boardEl.find('.' + CSS.square).removeClass(CSS.highlight_select);
            $('#' + SQUARE_ELS_IDS[select]).addClass(CSS.highlight_select);
        };

        widget.remove_highlight = function () {
            boardEl.find('.' + CSS.square).removeClass(CSS.highlight_select);
            boardEl.find('.' + CSS.square).removeClass(CSS.highlight_target);
            boardEl.find('.' + CSS.square).removeClass(CSS.highlight_source);
        };
        ...
        //new code
        widget.move_pieces = function (source, target) {
            DRAGGING_A_PIECE = true;
            DRAGGED_PIECE = CURRENT_POSITION[source];
            DRAGGED_PIECE_SOURCE = source;

            var action = 'drop';
            /*
            if (target === 'offboard' && cfg.dropOffBoard === 'snapback') {
                action = 'snapback';
            }
            if (target === 'offboard' && cfg.dropOffBoard === 'trash') {
                action = 'trash';
            }
            */
            ...
            //if (cfg.hasOwnProperty('onDrop') === true &&
                ...
                // source piece is a spare piece and position is off the board
                //if (DRAGGED_PIECE_SOURCE === 'spare' && target === 'offboard') {...}
                // position has not changed; do nothing

                // source piece is a spare piece and position is on the board
                if (DRAGGED_PIECE_SOURCE === 'spare' && validSquare(target) === true) {
                    // add the piece to the board
                    newPosition[target] = DRAGGED_PIECE;
                }

                // source piece was on the board and position is off the board
                if (validSquare(DRAGGED_PIECE_SOURCE) === true && target === 'offboard') {
                    // remove the piece from the board
                    delete newPosition[DRAGGED_PIECE_SOURCE];
                }
                ...
                var result = cfg.onDrop(DRAGGED_PIECE_SOURCE, target, DRAGGED_PIECE,
                    newPosition, oldPosition, CURRENT_ORIENTATION);
                if (result === 'snapback' || result === 'trash') {
                    action = result;
                }
            ...
            if (action === 'snapback') {
                ...
                dropDraggedPieceOnSquare(target);
            }
        };
