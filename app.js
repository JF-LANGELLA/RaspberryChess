...
app.post('/test/', function (req, res) {
    io.emit('rpi', { "userId": "paul", "gameId": users["paul"].games[0] });     // to communicate to the browser
});
*/

app.post('/test/', function (req, res) {
    data = req.body;
    //data.gameId = users[data.userId].games[0];
    data.gameId = user_gameId[data.userId];
    io.emit('rpi', data);     // to communicate to the browser
    res.status = 200;
    res.send("OK");
});
...
io.on('connection', function(socket) {
    ...
    socket.on('invite', function(opponentId) {
        ...
        user_gameId[opponentId] = game.id;
        user_gameId[socket.userId] = game.id;
